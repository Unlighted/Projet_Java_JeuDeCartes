...
/**
 * The strategy of the AI in case he's winning
 * @author Grenon Maxence Liu Runkai
 * 
 */
public class StrategyLosing implements Strategy{
	/**
    ...
    public int chooseCardToSwap(Prop exchangeProp, ArrayList<String> neededProp, Player otherPlayer1, Player otherPlayer2/* , ArrayList<Prop> seenImportantCards */) {
    	...
    	while(itAllProps.hasNext() && choice != 0) {
    		...
    		while(itNeededProps.hasNext() && choice != 0) { // checks if he sees the cards that he needs
    	...
    	while(itAllProps.hasNext())
    		if(itAllProps.next().getVisibility() == true)
    			counter++;
    	if(counter == 4) // if he sees everything chooses a random card
    		return r.nextInt(4)+1;
    	while(counter<100) { // returns an unseen cards
    ...
    public boolean chooseTurnTrick(TrickPile trickPile, ArrayList<Prop> ownedProps) {
    	...
    	compteur = ca+cb;
    	if(compteur == 0 || compteur == 2) // plays safe
    		return true;
    	return false;
    }
    ...
    public ArrayList<Prop> chooseRearrangeHand(ArrayList<Prop> listProp) {
    	...
    	Prop newOut = null;
    	while(seeAll.hasNext()) { //discard the least important card
    		buffer = seeAll.next();
    		if(buffer.getIsImportant()==false&&newOut==null)
    			newOut = buffer;
    		else
    ...
    public int chooseChangeVisibility(ArrayList<Prop> ownedProps) {
    	...
    	while(seeOwnedProps.hasNext()) { // shows an unimportant card
